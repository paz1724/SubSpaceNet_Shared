...
class SystemModel(object):
    ...
    def create_array(self):
        ...
        if self.sensors_array.sensors_array_form.startswith("Sparse"):
            self.virtual_array = self.actual_array
        else:
    ...
    def steering_vec_far_field(
        ...
        lambda_m = self.params.c / self.params.freq_Hz
        return (
                np.exp(
                    -2
                    * 1j
                    ...
                    * self.virtual_array
                    #* (np.array(self.virtual_array) / lambda_m)
                    * np.sin(theta)
                )
                + mis_geometry_noise
        )
